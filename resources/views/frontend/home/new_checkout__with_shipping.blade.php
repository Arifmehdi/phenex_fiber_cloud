                                ...
                                <div class="row">
                                    <div class="col-md-6 mt-3">
                                        <form>
                                            ...
                                            <div class="mb-3">
                                                <label for="billing-email" class="form-label">Email (Optional)</label>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="district" name="district" required style="font-size: 13px">
                                                    ...
                                                    @foreach($districts as $district)
                                                        <option value="{{ $district->id }}" @if($location && $location->district_id == $district->id) selected @endif>{{ $district->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="mb-3">
                                    ...
                                    <div class="accordion-body">
                                        ...
                                        <div class="tab-content" id="myTabContent">
                                            <div class="tab-pane fade show active" id="bkash" role="tabpanel" aria-labelledby="bkash-tab">
                                                <div class="card-body">
                                                    <p><strong>bKash Number:</strong> 01973-005566 (Personal)</p>
                                                </div>
                                            </div>
                                            <div class="tab-pane fade  " id="bank" role="tabpanel" aria-labelledby="bank-tab">
                                                <div class="card-body">
                                                    <p><strong>A/C Number:</strong> 1331070004347</p>
                                                    <p><strong>A/C Name:</strong> 93 Shasthoseba Pharma</p>
                                                </div>
                                            </div>

                                        </div>--}}
                                        <!-- <p class="mt-3">Make your payment directly into our bank or bKash account. Please use your Order ID as the payment reference. Your order will not be shipped until the funds have cleared in our account.</p> -->
                                        <p class="mt-3">Make your payment securely through the SurjoPay payment gateway.
                                                        You can complete your payment using bKash, card, or other supported methods.
                                                        Your order will be processed once the payment is successfully confirmed.</p>
                                    </div>
                                </div>
                            </div>
...
@push('js')
<script>
    const cartProductsData = @json($cartItems->map(fn($item) => ['free_shipping' => optional($item->product)->free_shipping])->values());
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const onlineRoute = "{{ route('spOrderStore') }}";
    ...
    // ✅ District change → load thanas and shipping methods
    ...
    function updateTotals() {
        ...
        if (subtotalElement && discountElement && payableElement && hiddenShippingPriceInput) {
            ...
            // Check if all products in cartProductsData have free_shipping = 1
            const allProductsFreeShipping = cartProductsData.every(product => product.free_shipping === 1);

            if (allProductsFreeShipping) {
                shippingCost = 0; // Free shipping if all products are free shipping
            } else {
                const selectedShipping = document.querySelector('input[name="shipping-option"]:checked');
                shippingCost = selectedShipping ? parseFloat(selectedShipping.value) : 0;
            }
